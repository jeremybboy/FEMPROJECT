...
public:
  ...
  int operator()(const  Simplex3 & tt) const  { return CheckT(&tt-t);}
  int operator()(const Vertex * vv)const  { return CheckV(vv-v);}  // (1)
  int operator()(const  Simplex3 * tt) const { return CheckT(tt-t);}
  Simplex3 & operator[](int k) { return t[CheckT(k)]; }
  const Simplex3 & operator[](int k) const { return t[CheckT(k)]; }
